...
def add_motion_blur(image):
    ksize = np.random.choice([3, 5, 7])  # Random kernel size
...
def add_shadow(image):
    ...
    top_y, bottom_y = 0, h
    overlay = image.copy()
    alpha = np.random.uniform(0.4, 0.6)  # Shadow intensity
...
# Define input explicitly for the Sequential model, with a unique name
...
# Add new dense layers for fine-tuning, skipping GlobalAveragePooling2D
...
epochs = 30  # Fine-tuning requires more epochs for refinement
history = model.fit(
    train_generator,
...
# Optional: Quantize the model for real-time deployment
